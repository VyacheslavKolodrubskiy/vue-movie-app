<template>
  <div v-if="movie" :key="movie.id" class="space-y-10">
    <div
    ...
    <YoutubeVideo :movie="movie" :title="title" />

    <h4 class="text-primary font-bold text-4xl">Similar {{ type }}</h4>
    <div
    ...
    >
      ...
      <MovieCardSkeleton v-else :quantity="20" />
    </div>

    <template v-if="actors.length">
      <h4 class="text-primary font-bold text-4xl">Actors</h4>
      <div
        class="grid grid-cols-4 gap-6 md:grid-cols-2 lg:grid-cols-3 xs:grid-cols-1"
      >
        <ActorCard
